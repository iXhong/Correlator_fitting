"""
Description: Jackknife fitting method for one-state & two-state model.
@author: GeorgeLiu
@since: 2025.11
"""
...
class JackknifeTwoStateFit(FitMethod):
    # jackknife fitting method for two-state model
    def __init__(self, model_func, residual_func, scale_factor):
        super().__init__(model_func, residual_func)
        self.scale_factor = scale_factor  # scale factor in two state fit

    def _init_params(self, data_fit) -> Parameters:
        # initialize fit parameters
        params = Parameters()
        params.add("A0", value=data_fit[0], min=0)
        params.add("A1", value=data_fit[1], min=0)
        params.add("m0", value=0.5, min=0)
    ...
    def _run_fit(
        ...
        # main jackknife fit routine for two-state model
        N_blocks = jk_samples.shape[0]
        ...
        for i_block in range(N_blocks):
            ...
            try:
                ...
                if result.success:
                    jk_params_A0.append(result.params["A0"].value / self.scale_factor)
                    jk_params_m0.append(result.params["m0"].value)
                    jk_params_A1.append(result.params["A1"].value / self.scale_factor)
        ...
        jk_params_A0 = np.array(jk_params_A0)
        jk_params_m0 = np.array(jk_params_m0)
        jk_params_A1 = np.array(jk_params_A1)
