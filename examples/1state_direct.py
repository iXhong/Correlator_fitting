...
mean,err = data_masked.mean(axis=0),data_masked.std(axis=0)

direct_fit = DirectFit(model_func=one_cosh_func,residual_func=residual)
result = direct_fit.fit(t_fit=t_masked,T=96,data_fit=mean,sigma=err,residual=residual,print_report=True)
