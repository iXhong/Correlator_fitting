...
# 诊断：比较三种 time-point error estimator
def compare_point_errors(t_min, t_max, t, data, n_boot=50, seed=1234):
    fit_mask = (t >= t_min) & (t <= t_max)
    t_fit = t[fit_mask]
    N = data.shape[0]

    # 1) 原始样本标准差 / sqrt(N)  (常见的 mean stderr)
    sample_se = np.std(data[:, fit_mask], axis=0, ddof=1) / np.sqrt(N)

    # 2) jackknife (按照你现有代码计算)
    jk_means = []
    for i in range(N):
        idx = np.concatenate([np.arange(i), np.arange(i+1, N)])
        jk_means.append(np.mean(data[idx][:, fit_mask], axis=0))
    jk_means = np.array(jk_means)
    jk_sigma = np.sqrt((N-1)/N * np.sum((jk_means - np.mean(jk_means, axis=0))**2, axis=0))
    ...
    for _ in range(n_boot):
        ...
        bs_means.append(np.mean(data[chosen][:, fit_mask], axis=0))
    bs_means = np.array(bs_means)
    bs_sigma = np.std(bs_means, axis=0, ddof=1)

    plt.figure()
    plt.scatter(t_fit,np.log(sample_se))
    plt.scatter(t_fit,np.log(jk_sigma))
    plt.scatter(t_fit,np.log(bs_sigma))
    plt.show()

    # print("t_fit:", t_fit)
    # print("sample_se:", sample_se)
    # print("jk_sigma:", jk_sigma)
    # print("bs_sigma:", bs_sigma)

# 在 main 中调用:
# compare_point_errors(tmin, tmax, t, data, n_boot=500)



def main(tmin, tmax, show_plot, print_report, n_resamples=1000):
    data, t, _ = data_load()
    ...
    compare_point_errors(tmin, tmax, t, data, n_boot=50)
...
if __name__ == "__main__":
    main(tmin=5, tmax=44, show_plot=False, print_report=True, n_resamples=10)
